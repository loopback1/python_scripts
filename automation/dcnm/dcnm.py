...
class Session(object):
    ...
    def login(self):
        ...
        try:
            resp = requests.post(url, auth=(
                self.user, self.passwd), data=json.dumps(payload))
            if resp.ok:
                logging.info('Successfully logged into %s' % url)
            else:
                logging.error('Could not login to %s: Response: %s' %
                              (url, resp.text))
    ...
    def get(self, url):
        ...
        if resp.ok:
            ...
            logging.error(
                'Cloud not get %s. Received response: %s', url, resp.text)
        return resp
    ...
    def save_settings(self, obj):
        if isinstance(obj, AutoConfigSettings):
            resp = requests.put(self.base_url + '/auto-config/settings',
                                headers=self.headers, data=obj.get_json())
            return resp
        else:
            raise TypeError()
